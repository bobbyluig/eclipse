...
namespace ORB_SLAM2
{
		...
		mpORBVocabulary(&pVoc), mpMap(&pMap), mpKeyFrameDB(&pKFDB), mpInitializer(static_cast<Initializer*>(NULL))
	{
		...
		float cx = fSettings.cx;
		float cy = fSettings.cy;
		...
		DistCoef.at<float>(0) = fSettings.k1;
		DistCoef.at<float>(1) = fSettings.k2;
		DistCoef.at<float>(2) = fSettings.p1;
		DistCoef.at<float>(3) = fSettings.p2;
		const float k3 = fSettings.k3;
		if (k3 != 0)
		{
			DistCoef.resize(5);
		...
		int nRGB = fSettings.RGB;
		mbRGB = nRGB;

		// Load ORB parameters

		int nFeatures = fSettings.nFeatures;
		float fScaleFactor = fSettings.fScaleFactor;
		int nLevels = fSettings.nLevels;
