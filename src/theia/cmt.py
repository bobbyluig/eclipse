...
class CMT(object):
    ...
    @staticmethod
    def inRectangle(keypoints, tl, br):
        if type(keypoints) is list:
            keypoints = CMT.convertKeyPoints(keypoints)
    ...
    @staticmethod
    def convertKeyPoints(keypoints_cv):
    ...
    def track(im_prev, im_gray, keypoints, THR_FB=20):
        ...
        if num_keypoints > 0:
            ...
            pts_back = CMT.squeezePoints(pts_back)
            pts = CMT.squeezePoints(pts)
            nextPts = CMT.squeezePoints(nextPts)
    ...
    def estimate(self, keypoints):
        ...
        if keypoints.size > 1:
            ...
            if not all(duplicate_classes):
                ...
                dists = CMT.L2Norm(pts_allcombs0 - pts_allcombs1)
                ...
                angle_diffs[long_way_angles] = angle_diffs[long_way_angles] - \
                                               np.sign(angle_diffs[long_way_angles]) * 2 * math.pi
                ...
                votes = keypoints[:, :2] - scale_estimate * (CMT.rotate(self.springs[keypoint_class - 1], med_rot))
    ...
    def process_frame(self, im_gray):
        ...
        tracked_keypoints, _ = CMT.track(self.im_prev, im_gray, self.active_keypoints)
        ...
        if len(keypoints_cv) > 0:
            transformed_springs = scale_estimate * CMT.rotate(self.springs, -rotation_estimate)
            for i in range(len(keypoints_cv)):
                ...
                if not any(isnan(center)):
                    ...
                    displacements = CMT.L2Norm(transformed_springs - relative_location)
        ...
        if not any(isnan(self.center)) and self.active_keypoints.shape[0] > self.num_initial_keypoints / 10:
            ...
            tl = CMT.array2Tuple(center + scale_estimate * CMT.rotate(self.center_to_tl[None, :],
                                                                      rotation_estimate).squeeze())
            tr = CMT.array2Tuple(center + scale_estimate * CMT.rotate(self.center_to_tr[None, :],
                                                                      rotation_estimate).squeeze())
            br = CMT.array2Tuple(center + scale_estimate * CMT.rotate(self.center_to_br[None, :],
                                                                      rotation_estimate).squeeze())
            bl = CMT.array2Tuple(center + scale_estimate * CMT.rotate(self.center_to_bl[None, :],
                                                                      rotation_estimate).squeeze())
