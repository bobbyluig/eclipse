...
def absoluteColor(hue, mission=True):
    """
...
def m1(image):
    ...
    hue = averageChannel(image)
    return absoluteColor(hue * 2)


def m2(image):
    colors = clusterColor(image)
    return absoluteColor(colors[0][0][0])
...
def m4(image):
    means = clusterColor(image, type='lab')
...
image = cv2.imread('sand8.jpg')

for i in range(1, 5):
    start = time.time()

    for j in range(10):
        result = locals()['m' + str(i)](image)

    delta = (time.time() - start) / 10 * 1000

    print('Method %s: %s in %s ms' % (i, result, delta))
